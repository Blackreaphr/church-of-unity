export default {
  async fetch(request, env) {
    try {
       ...
       * @returns {string}
       */
      function cookieHeader(name, value, { https, partitioned } = /** @type {{ https?: boolean, partitioned?: boolean }} */ ({}) ) {
      ...
      if (service === 'forum') {
        ...
        if (action === 'create') {
          ...
          const client = {
            ...
            rule_hits: post.rule_hits,
            author_secret,
            view_url: `/forum/post.html?id=${encodeURIComponent(id)}`
          };
        ...
        if (action === 'feed') {
          ...
          for (const k of list.keys) {
            ...
            try {
              ...
              items.push({ id: p.id, title: p.title, category: p.category || '', tags: p.tags || [], created_at: p.created_at, excerpt: excerpt(p.body, 240), url: `/forum/post.html?id=${encodeURIComponent(p.id)}` });
            } catch {}
          }
